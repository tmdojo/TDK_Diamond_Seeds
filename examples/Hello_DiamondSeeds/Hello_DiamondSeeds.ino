  ...
  if (slideSwitch_i) {
    while(Check2){
      ...
      if(!slideSwitch){
        ...
        Check2 = false;
        Serial.println("Puzzle2:slide switch");
      }
    }
  } else {
    while(Check2){
...
//Puzzle3:use gyrosensor 
  ...
  while(Check3){
    ...
    if(abs(Gx+Gy+Gz-Gx_i-Gy_i-Gz_i)>500){
      ...
      Check3 = false;
      Serial.println("Puzzle3:use gyrosensor");
    }
  }

//Puzzle4:use right button 
  while(Check4){
    if(TDSs.rightButton()){
      TDSs.setPixelColor(3, 255,   0,   0);      
      Check4 = false;
      Serial.println("Puzzle4:use rightButton");          
    }
  }  

//Puzzle5:use thermistor 
  ...
  while(Check5){
    ...
    if(abs(Tt-Tt_i)>2){
      ...
      Check5 = false;
      Serial.println("Puzzle5:use thermistor");      
    }
  }

//Puzzle6:use MEMS microphone 
  ...
  while(Check6){
    ...
    if(abs(dB-dB_i)>7500){
      ...
      Check6 = false;
      Serial.println("Puzzle6:use MEMSmicrophone");           
    }
  }
